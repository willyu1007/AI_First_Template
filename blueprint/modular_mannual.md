...
This modular specification is the structural spine of the repository. It relies on a small set of companion documents for specific mechanisms:

- `ability_routing.md` – ability pool, `ABILITY.md` structure, and the standard execution script.
- `content_routing.md` – knowledge routing via `ROUTING.md` and topic route files.
- `agents_strategy_guide.md` – how `AGENTS.md` files are organized and interpreted.
- `hooks.md` – event-driven hook system around prompts and ability calls.
- `devops_extension_guide.md` – initialization, database mirrors, packaging, and deployment.

For an AI developer, the recommended high-level reading order is:

- **Single-module work**: this spec → module-level `AGENTS.md` → module `ROUTING.md` → module `ABILITY.md` → `workdocs/` for the current task.
- **Cross-module work**: this spec → `modules/AGENTS.md` → `modules/overview/AGENTS.md` and registries → `modules/integration/AGENTS.md` and integration docs.
- **DevOps scenarios**: this spec → `devops_extension_guide.md` → the `AGENTS.md` inside `/PROJECT_INIT`, `/db`, `/ops/packaging`, or `/ops/deploy`.
