...
This guide follows the shared glossary defined in the repository root README (`AI developer`, `internal orchestration`, `runtime`, `hook layer`, and `humans`). It uses those terms as defined there and does not introduce additional role names.
...
This document should stay aligned with the modular repository layout and terminology described in `modular_mannual.md`, including module types, module instances, and the type graph.
...
**Location**: module instances expose `AGENTS.md` at `modules/<module_id>/AGENTS.md`; other important directories (for example `/db`, `/ops/packaging`, `/ops/deploy`, or `workdocs/`) may also provide their own `AGENTS.md`.
...
When a task clearly spans multiple modules, `/modules/AGENTS.md` should route the AI developer to:

- `modules/overview/AGENTS.md` and the registries under `modules/overview/` for type graphs, instance lists, and interface definitions.
- `modules/integration/AGENTS.md` and related files for integration and end-to-end scenarios.
- The `AGENTS.md` files of the specific modules involved.

This reading order matches the modular specification and keeps cross-module work predictable.
...
### 5.3 Example: Global vs module-level rules

Consider the following situation:

- The project root `AGENTS.md` states: “Do **not** modify production deployment configuration files under `/ops/deploy/prod/`.”
- The `AGENTS.md` for `modules/payments.api/` states: “You **may** adjust staging deployment configs for this module under `/ops/deploy/staging/payments-api/`.”

When working on the payments API deployment:

- Editing staging configs is allowed, because it is explicitly in scope for the module-level strategy.
- Editing production configs remains forbidden, because the global rule applies and there is no local override.
- If a local rule ever appears to conflict with a global one, the AI should treat the global restriction as dominant and prefer the safer interpretation.

---

### 5.4 Handling Conflicts and Uncertainty
