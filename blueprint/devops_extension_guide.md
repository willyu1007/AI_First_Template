...
### 2.6 Strategy docs (`AGENTS.md`) for DevOps areas

For each DevOps-focused directory, it is recommended to add a short, local `AGENTS.md` that explains:
...
Where these scripts are naturally aligned with repository events (for example, after a session ends or after a migration ability runs), projects may use hooks (see `hooks.md`) to trigger lightweight checks or summaries instead of wiring every DevOps step directly into prompts.
...
```
In addition to this central `/db/` area, each module instance documents its own data usage under `modules/<module_id>/interact/db/` as described in `modular_mannual.md`. Module-level docs focus on which tables and operations a module owns or touches, while the `/db/` tree provides the project-wide schema, migrations, and environment configuration. Changes to schemas or migrations should keep these two views consistent.
...
```
Deployment descriptors should also stay linked to module instances and interfaces. Where relevant, they can reference entries in `modules/overview/interfaces.yaml` and `modules/overview/api_gateway.yaml` so that it is clear which HTTP routes or workloads are being deployed. Module-level docs may in turn link to the matching files under `/ops/deploy/` to make environment expectations discoverable.
