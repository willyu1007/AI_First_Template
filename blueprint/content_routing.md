...
- **Contextual / ephemeral documents**  
  ...
  - See `modular_mannual.md` (Modular Development Specification) for details.
...
This specification assumes a **hybrid layout**:
- Each module instance normally acts as its own routing root.
- Optionally, the repository may also expose project- or integration-level routing roots for fully cross-module topics.
...
#### Project-level routing roots (optional)
...
In practice, these structured intents often flow into the hook system: a `PromptSubmit` hook can translate a trigger’s intent into `routing_hint` and `normalized_intent` hook signals (see `hooks.md`), so that the AI developer receives them as part of the turn context.
