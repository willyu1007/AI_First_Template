...
1. 完成 **Ability Runtime（ability_runner）**：  
...
1. **Ability Runtime（ability_runner）**  
...
## 4. Ability Runtime（ability_runner）设计

### 4.1 目标

Ability Runtime 为外部世界提供一个“任务中心”的抽象，使 AI 和其他调用方可以：
...
同时，Ability Runtime 是 “标准执行脚本” 的实现者，必须符合 ability_routing 文档中对 Create task / Run task / Direct run / Query/modify 的描述。

### 4.2 对外接口

建议能力运行库（`scripts/runtime/ability_runner.py`）提供如下 CLI/API 接口：
...
- 通用 usage / telemetry（例如所有能力调用的计数、时延统计）统一由 **Ability Runtime 内部**完成；  
...
- 形态：一个由 Agent Runtime 调用的库（脚本/模块），**不是必须的长驻进程**；  
- 主要职责：  
